  ...
  return (
    <div className="min-h-[100dvh] w-full flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      >
        ...
        <form onSubmit={handleJoin} className="tactical-panel p-8 flex flex-col gap-6 relative">
          ...
          {roomId && roomId.length === 4 && (
            <div className="mt-6 pt-6 border-t border-cyan-500/10 w-full flex flex-col items-center">
              <p className="text-[7px] font-mono text-cyan-500/20 uppercase tracking-[0.2em] mb-2">System_Corrupted?_Manual_Override:</p>
              <button 
                type="button"
                onClick={() => {
                  if (confirm(`ALERT: This will wipe all data for sector ${roomId}. Proceed with caution?`)) {
                     onReset(roomId);
