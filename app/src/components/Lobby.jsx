...
export default function Lobby({ gameState, playerId, onStart, onAddBot }) {
	...
	const canAddBot = gameState.players.length < 10;
	...
	return (
		<div className="h-[100dvh] w-full flex flex-col p-4 sm:p-6 pt-16 sm:pt-14 relative overflow-hidden">
			...
			<div className="flex items-center justify-center gap-2 mb-2 text-[10px] font-mono text-cyan-500/60 uppercase tracking-[0.35em] shrink-0 pointer-events-none">
				<span>ROOT</span>
				<span className="opacity-30">//</span>
				<span>DECRYPT</span>
				<span className="opacity-30">//</span>
				<span className="text-cyan-400 font-bold">SECTOR_LOBBY</span>
			</div>
			...
			>
				...
				<p className="mt-3 text-[11px] leading-relaxed text-cyan-100/55 normal-case tracking-[0.08em] max-w-xs mx-auto">
					Eclipse is optimized for clandestine operations. Deployed operatives should maintain strict screen discipline for maximum effectiveness.
				</p>
			</motion.div>
				...
				<div className="flex items-center justify-between mb-3 border-b border-cyan-500/20 pb-2 overflow-hidden shrink-0">
					<h3 className="text-xs sm:text-sm font-mono tracking-widest text-cyan-400 uppercase flex items-center gap-2">
						...
						<span className="flex items-center">
							...
							<span className="ml-1"> / 10 OPERATIVES</span>
						</span>
					</h3>
				</div>
							...
							>
								<div className="flex items-center gap-3 min-w-0">
									<span className={`font-mono tracking-widest uppercase text-xs sm:text-sm truncate flex items-center gap-2 ${p.id === playerId ? 'text-white font-bold' : 'text-white/80'}`}>
										{p.name}
										{p.isBot && <span className="text-cyan-400/60 text-[8px] border border-cyan-500/20 bg-cyan-950/40 px-1 py-0.5 font-bold">SYNTH_SYS</span>}
			...
			<div className="fixed bottom-0 inset-x-0 bg-gradient-to-t from-[#010305] via-[#010305] to-transparent z-50 flex flex-col justify-end pointer-events-none pb-8 pt-16 px-4 sm:px-6">
				<div className="max-w-sm mx-auto w-full flex flex-col gap-0 pointer-events-auto">
					...
					<div className="flex flex-col border border-cyan-500/30 bg-black/80 backdrop-blur-md overflow-hidden rounded-sm shadow-[0_20px_50px_rgba(0,0,0,1)]">
						...
						{/* A. DEPLOY BOT button (Host only) */}
						{isHost && (
							<motion.button
								whileTap={{ scale: 0.98 }}
								onClick={onAddBot}
								disabled={!canAddBot}
								className={`relative w-full py-3 h-[50px] font-mono text-[10px] tracking-[0.3em] font-bold uppercase transition-all duration-300 border-b border-cyan-500/20 px-6
                  ${canAddBot ? 'text-cyan-400 hover:bg-cyan-950/30' : 'text-cyan-900 opacity-20'}`}
							>
								<div className="flex items-center justify-center gap-2">
									<Users size={12} className="opacity-60" />
									CONFIRM_BOT_OPERATIVE
								</div>
							</motion.button>
						)}

						{/* B. Contextual Primary Action */}
						{isHost ? (
							<motion.button
								...
								onClick={onStart}
								disabled={!canStart}
								className={`relative w-full py-4 font-mono transition-all duration-500 flex flex-col justify-center items-center px-6 border-b border-cyan-500/10
                  ${canStart
									...
									<AnimatePresence mode="popLayout">
										{canStart ? (
											...
											<motion.span
												...
												exit={{ y: -10, opacity: 0 }}
												className="text-[10px] tracking-widest font-bold text-cyan-500/80 uppercase block"
											>
							...
							<div className={`w-full backdrop-blur-md py-4 flex items-center justify-center flex-col gap-1.5 uppercase tracking-[0.2em] text-[10px] font-mono font-black transition-colors duration-500 border-b border-cyan-500/10 ${canStart ? 'bg-cyan-950/40 text-cyan-400' : 'bg-black/60 text-cyan-500/30'}`}>
								<div className="flex items-center gap-2">
									...
									SUPERVISOR IS SECURING THE SECTOR
								</div>
						...
						{/* C. Secondary Action (Share) */}
						<AnimatePresence>
							<motion.button
								layout
								whileTap={{ scale: 0.985 }}
								onClick={() => handleCopy(`${window.location.origin}?room=${gameState.roomId}`, 'link')}
								className={`relative w-full h-[54px] flex items-center justify-center gap-2.5 px-6 text-[11px] font-mono font-bold tracking-[0.25em] uppercase transition-all
                  ${copiedLink
										? 'text-cyan-500 bg-cyan-950/40'
										: 'text-cyan-600 bg-transparent hover:text-cyan-300 hover:bg-cyan-950/10'}
                  ${canStart && !copiedLink ? 'opacity-50 hover:opacity-100' : ''}
                `}
							>
								<Share size={18} className={copiedLink ? 'hidden' : 'opacity-70'} />
								{copiedLink ? <Check size={18} className="text-cyan-400" /> : null}
								<span className="mt-[2px]">{copiedLink ? 'UPLINK_COPIED' : 'BROADCAST_ACCESS_UPLINK'}</span>
