...
// Cyclic text component for Scanning phase
const ScanningText = () => {
  const texts = ["SCANNING_IDENTITY...", "VERIFYING_CLEARANCE...", "DECRYPTING_ROLE..."];
  const [idx, setIdx] = useState(0);

  useEffect(() => {
    const t = setInterval(() => setIdx(i => (i + 1) % texts.length), 500);
    ...
    <div className="relative w-full h-12 flex items-center justify-center overflow-hidden">
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="absolute font-mono text-cyan-400 text-xs tracking-widest uppercase font-black"
        >
          {texts[idx]}
        </motion.div>
      </AnimatePresence>
      ...
      <motion.div
        ...
        className="absolute top-0 bottom-0 w-8 bg-gradient-to-r from-transparent via-cyan-400/30 to-transparent skew-x-12"
      />
    </div>
  );
};
...
export default function RoleReveal({ gameState, playerId, onReady }) {
  ...
  const showCardContent = state !== REVEAL_STATES.IDLE && state !== REVEAL_STATES.AUTH_TAP && state !== REVEAL_STATES.SCANNING;
  ...
  return (
    <div className="h-[100dvh] w-full flex flex-col px-4 sm:px-6 pb-4 sm:pb-6 pt-20 bg-obsidian-900 overflow-hidden relative">
      ...
      <motion.div 
        ...
        className="absolute inset-0 pointer-events-none bg-[url('/grid.svg')] opacity-10"
      />
      ...
      >
        <h2 className="text-xl sm:text-2xl font-mono text-cyan-400 tracking-[0.25em] font-black neon-text-cyan uppercase">Identity_Verify</h2>
        <p className="text-cyan-500/60 text-[9px] mt-1 font-mono uppercase tracking-[0.4em] font-bold">// LEVEL 5 CLEARANCE ONLY</p>
      </motion.div>
          ...
          <AnimatePresence mode="wait">
            ...
            {(state === REVEAL_STATES.IDLE || state === REVEAL_STATES.AUTH_TAP) && (
              ...
              >
                ...
                <p className="text-cyan-500/80 text-[11px] uppercase tracking-[0.25em] font-mono font-bold animate-pulse">Tap to Authenticate</p>
              </motion.div>
            )}

            {/* 2. SCANNING */}
            {state === REVEAL_STATES.SCANNING && (
              <motion.div 
                key="scanning"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              ...
              >
                <div className="w-full h-1 bg-cyan-900/40 rounded-full mb-8 overflow-hidden relative">
                   <motion.div 
                     initial={{ x: '-100%' }} animate={{ x: '100%' }} transition={{ repeat: Infinity, duration: 1, ease: 'linear' }}
                     className="absolute inset-y-0 w-1/3 bg-cyan-400"
                   />
                </div>
                <ScanningText />
              </motion.div>
            ...
            {/* 3. REVEAL SEQUENCE (DECRYPT -> CONFIRMED) */}
            {showCardContent && (
              <motion.div 
                key="content"
                initial={{ opacity: 0 }}
              ...
              >
                ...
                <AnimatePresence>
                  {(state === REVEAL_STATES.DETAILS || state === REVEAL_STATES.OPERATIVES || state === REVEAL_STATES.CONFIRMED) && !privacyMode && (
                     ...
                     >
                       ...
                       {isFactionLiberal 
                         ? 'Find your fellow Liberals. Pass 5 Liberal policies or assassinate Hitler.' 
                         : 'Stay hidden. Pass 6 Fascist policies or elect Hitler as Chancellor after 3 Fascist policies.'}
                     </motion.div>
                  )}
                </AnimatePresence>
                ...
                <AnimatePresence>
                  {(state === REVEAL_STATES.OPERATIVES || state === REVEAL_STATES.CONFIRMED) && visibleTeammates.length > 0 && !privacyMode && (
                    ...
                    >
                      <p className="text-[8px] sm:text-[9px] text-white/40 font-mono uppercase tracking-[0.3em] mb-2 border-b border-white/10 pb-1 font-bold">Known Operatives</p>
      ...
      <div className="flex justify-center mt-auto pt-2 shrink-0 px-2 pb-2 h-[80px]">
        <AnimatePresence>
          {state === REVEAL_STATES.CONFIRMED && (
            ...
            >
              <span className="opacity-60 mr-2">{'>'}</span> ACKNOWLEDGE_DIRECTIVE
            </motion.button>
          )}
        </AnimatePresence>
      </div>
